...
class TreatmentProtocol(KnowledgeEngine):
    ...
    def laba_lama_exacerbations_low_eosinophils(self):
        self.reatment_protocol_res = "Bệnh nhân còn đợt cấp với bạch cầu ái toan < 100 tế bào/µL. Khuyến cáo thêm Roflumilast hoặc Azithromycin."
        print(treatment_protocol_result)
        self.declare(TreatmentData(treatment_protocol_result=treatment_protocol_result))
...
def input_treatment_data():
    ...
    symptoms = input("Triệu chứng hiện tại (persistent/none): ").strip().lower()
    ...
    exacerbations = input("Tình trạng đợt cấp (persistent/none): ").strip().lower()
